...
for i=1:m
   Y(y(i),i)=1;
   delta3=a3(:,i)-Y(:,i);%10x1
   delta2=Theta2(:,[2:end])'*delta3.*sigmoidGradient(z2(:,i));%25x1
   D1=D1+delta2*a1(:,i)';
   D2=D2+delta3*a2(:,i)';
endfor
D1=D1/m;
...
t10=Theta1(:,1);
t20=Theta2(:,1);
J=-sum(sum(Y.*log(a3)+(1-Y).*log(1-a3)))/m+lambda*(sum(nn_params.*nn_params)-sum(t10.*t10)-sum(t20.*t20))/(2*m);
